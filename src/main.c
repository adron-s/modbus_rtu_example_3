...
uint8_t uart2_rx_dma_buf[UART2_RX_DMA_BUF_SIZE]; // DMA circular buffer
volatile uint16_t uart2_rx_dma_last_pos = 0; // last processed position

nmbs_t nmbs; // nanomodbus library instance
const uint8_t nmbs_id = 0x01; // modbus ID of this device

int loop_count = 0;

void modbus_server_init(void) { nmbs_server_init(&nmbs, nmbs_id); }
...
/* Test function for debugging UART DMA operations. */
// void uart_dma_process(void) {
// 	uint16_t dma_pos = UART2_RX_DMA_BUF_SIZE -
// __HAL_DMA_GET_COUNTER(huart2.hdmarx);

// 	if (uart2_rx_dma_last_pos != dma_pos) {
// 		printf("%s::data>", __func__);

// 		do {
// 			uint8_t b = uart2_rx_dma_buf[uart2_rx_dma_last_pos];
...
// 			}
// 		} while (uart2_rx_dma_last_pos != dma_pos);
...
// __HAL_DMA_GET_COUNTER(huart2.hdmarx)); 		uart_dma_process();
// 		// HAL_StatusTypeDef status = HAL_UART_Receive(&huart2, buf,
// count, 100);
// 		// if (status == HAL_OK) {
// 		// 	printf("%s::data> ", __func__);
...
static void MX_USART1_UART_Init(void)
{
	...
	GPIO_InitStruct.Alternate =
	    GPIO_AF1_USART1; // This is the "secret sauce" for AF
...
static void MX_USART2_UART_Init(void)
{
	...
	GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_HIGH;
	GPIO_InitStruct.Alternate =
	    GPIO_AF1_USART2; // This is the "secret sauce" for AF
	HAL_GPIO_Init(GPIOA, &GPIO_InitStruct);
	...
	/* Enable DMA. */
	HAL_UART_Receive_DMA(&huart2, uart2_rx_dma_buf, UART2_RX_DMA_BUF_SIZE);
	/* Force DMA into circular mode. */
	__HAL_DMA_DISABLE_IT(huart2.hdmarx,
			     DMA_IT_HT); // optional: disable half-transfer IRQ
	huart2.hdmarx->Instance->CCR |= DMA_CCR_CIRC; // enable circular mode
}

/*
	DMA Initialization for:
		USART2
*/
void MX_DMA_Init(void)
{
...
void SystemClock_Config(void)
{
	...
	RCC_OscInitStruct.OscillatorType =
	    RCC_OSCILLATORTYPE_HSI; // use Internal High Speed
	RCC_OscInitStruct.HSIState = RCC_HSI_ON; // turn it ON
	RCC_OscInitStruct.HSICalibrationValue = RCC_HSICALIBRATION_DEFAULT;
	// RCC_OscInitStruct.PLL.PLLState = RCC_PLL_NONE; // using HSI directly
	// - don't use the Multiplier (PLL)
	RCC_OscInitStruct.PLL.PLLState =
	    RCC_PLL_ON; // change PLLState to ON - for 48Mhz clock !
	...
	/*
		...
		HCLK (AHB Bus): Controls high-speed peripherals and memory
	   access. PCLK1 (APB Bus): Controls slower peripherals like GPIOs,
	   Timers, and UART.
	*/
	RCC_ClkInitStruct.ClockType =
	    RCC_CLOCKTYPE_HCLK | RCC_CLOCKTYPE_SYSCLK | RCC_CLOCKTYPE_PCLK1;
	// RCC_ClkInitStruct.SYSCLKSource = RCC_SYSCLKSOURCE_HSI; // set System
	// clock to HSI (8Mhz) - as the main System Clock
	//  Change Source from HSI to PLLCLK
	RCC_ClkInitStruct.SYSCLKSource =
	    RCC_SYSCLKSOURCE_PLLCLK; // set System clock to PLL - for 48Mhz
				     // clock !
	...
	RCC_ClkInitStruct.AHBCLKDivider =
	    RCC_SYSCLK_DIV1; // don't slow down AHB
	RCC_ClkInitStruct.APB1CLKDivider = RCC_HCLK_DIV1; // don't slow down APB
	HAL_RCC_ClockConfig(
	    &RCC_ClkInitStruct,
	    FLASH_LATENCY_0); // use Latency 0 for maximum flash memory speed.
}
...
static void MX_GPIO_Init(void)
{
	...
	/* In an STM32, every peripheral (GPIO, Timers, UART) is turned off by
	   default. If you try to write to a pin without enabling its "Clock"
	   (the power/timing signal for that port), the command will be ignored.
	   This line wakes up Port A. */
	__HAL_RCC_GPIOA_CLK_ENABLE();
	__HAL_RCC_GPIOB_CLK_ENABLE();

	/* Configure GPIO pin Output Level */
	/* This ensures the LED is OFF (Reset) the very moment the pin is
	   activated. Without this, the pin might "glitch" or stay in an unknown
	   state until your while(1) loop starts. */
	HAL_GPIO_WritePin(LED_GPIO_PORT, LED_PIN, GPIO_PIN_RESET);
